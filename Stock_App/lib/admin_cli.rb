...
class Admin_cli
  ...
  def admin_main
    ...
    loop do
      ...
      when "modify users"
          edit_users_menu
      when "reseed database"
        puts "SEEDING DATABASE CANNOT BE UNDONE ARE YOU SURE? [YES]/[NO]"
        input = gets.chomp.downcase
        case input
        when "yes"
            Run.seed
  ...
  def edit_users_menu
    ...
    puts "What would you like to do to user? [change name] [change balance] [view transactions] [view balance]"
    input = gets.chomp.downcase
    ...
    when "view balance"
      puts "Balance is $#{User_cli.float_to_cash(user.cash_balance)}"
    end
