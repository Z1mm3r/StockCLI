...
class User_cli
  ...
  def detail
    ...
    puts "#{info["companyName"]} (#{info["symbol"]}) Sector: [#{info["sector"]}] trading at $#{User_cli.float_to_cash(info["latestPrice"])}. Today's high: #{User_cli.float_to_cash(info["high"])}. Today's low: #{User_cli.float_to_cash(info["low"])}."
  end
